...
printf '%s\n' "$recipient" > "$FAKE_RECORD_DIR/age-recipient.txt"
cp "$input" "$output"
BASH
chmod +x "$tmp/bin/certbot" "$tmp/bin/age"
...
run_renewal() {
  ...
  PFX_AGE_RECIPIENT="age1testrecipient" \
  PATH="$tmp/bin:$PATH" \
    scripts/renew-cert.sh "$domain" "$1" "$2"
}

run_renewal "$tmp/public.pem" "$tmp/certificate.pfx.age" > "$tmp/renewal.out"
...
assert_eq "age1testrecipient" "$(<"$tmp/age-recipient.txt")" "age recipient"
...
openssl pkcs12 -in "$tmp/certificate.pfx.age" -passin pass: -info -noout >/dev/null 2>&1 \
...
if FAKE_CERT_SOURCE="$tmp/wrong/cert.pem" \
   ...
   run_renewal "$tmp/existing.pem" "$tmp/wrong.pfx.age" >/dev/null 2>&1; then
...
[[ ! -e "$tmp/wrong.pfx.age" ]] || fail "wrong-SAN encrypted artifact exists"

if FAKE_KEY_SOURCE="$tmp/unrelated/privkey.pem" \
   run_renewal "$tmp/existing.pem" "$tmp/mismatch.pfx.age" >/dev/null 2>&1; then
...
if FAKE_FULLCHAIN_SOURCE="$tmp/source/cert.pem" \
   run_renewal "$tmp/existing.pem" "$tmp/leaf-only.pfx.age" >/dev/null 2>&1; then
